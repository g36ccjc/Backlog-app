...
export default async function handler(req, res) {
  ...
  if (req.query?.recent === "1") {
    ...
    if (!key) return res.status(500).json({ error: "Server missing STEAM_API_KEY." });
    try {
      // Ground truth first: the "Recent Activity" box on the user's own
      // community profile — the exact games, in the exact order, Steam
      // itself shows. The recently-played API only decorates with hour
      // counts (its ordering has proven unreliable).
      ...
      if (profR.status === "fulfilled" && profR.value) {
        const blocks = profR.value.split(/class="recent_game"/).slice(1, 5);
        for (const b of blocks) {
          const idM = b.match(/\/app\/(\d+)/);
          if (!idM) continue;
          const appid = +idM[1];
          if (games.some((g) => g.appid === appid)) continue;
          const nameM = b.match(/class="game_name">\s*<a[^>]*>([^<]+)</);
          const lpM = b.match(/last played on ([^<\n]+)/i);
          ...
          games.push({
            ...
            lastPlayedText: lpM ? lpM[1].trim()
              : (/currently in-game/i.test(b) ? "In-game now" : null),
          });
        }
        ordered = games.length > 0;
      }

      // Fallback: profile private or markup changed — API set, API order
      if (!games.length) {
